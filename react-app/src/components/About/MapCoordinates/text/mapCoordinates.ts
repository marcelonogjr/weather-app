export const mapCoordinatesText = [
  ...
  was to centralize the called coordinates. But to understand _why_ this was a problem, I should first 
  ...
  the equator line appear larger than they really are.
`,
  `
  As you can see in the representation above, the lines are separated representing 15º of the globe apart from one 
  another, but while the longitude (vertical lines) have a constant distance from each other, the latitude 
  (horizontal lines) distance increases significantly the further they are from the _E.L._ (Equator Line - 0° latitude).

  That distortion makes the north and south poles appear at a point that distances _infinitely_ from the _E.L._, so most
  maps cover up to 89° of latitude (north and south). In fact, most maps used in navigation apps use the so-called 
  [Web Mercator Projection](https://en.wikipedia.org/wiki/Web_Mercator_projection), which uses a slightly 
  [different concept of latitude](https://en.wikipedia.org/wiki/Geodetic_coordinates) and limits it to _85.051129°_, 
  so that the boundaries of the projection form a perfectly squared image and sets a standard of 256px of width and
...
`,
  ...
  system references an area (always a square with 256px of side length) using 3 variables (_x_, _y_, _zoom_), and it
  ...
  and _y_ variables map the position of the tiles in the horizontal and vertical axis, respectively, as follows.
`,
  `
  So now that we understand how the projection and the tile coordinate system work, I can finally explain the main
  problem and how I solved it.
`,
  `
  As you can probably imagine, there's a bit of a problem with the tiles coordinate system in a weather app context,
...
`,
  ...
  Let's take the San Francisco illustration above, with _zoom = 10_ and (_x_, _y_) = (_163_, _395_) - this example will
  be used repeatedly in this article to make the comprehension of the solution easier. In order to complete that
  image, since our **_I_ _point_** (in this case) is located in the bottom-right corner of the image, it is required
  to take the directly right, directly down and right-down diagonal tiles - (_164_, _395_), (_163_, _396_) and
  (_164_, _396_), respectively.

  So, in order to create a general rule that may be applicable to every location and different
  zoom levels, it's necessary to follow these steps:
...
`,
  ...
  As you can see, the formula to find _x_ is very straightforward, but to find _y_ with a given _lat_ can be more 
...
`,
  ...
  To find out in what region the **_I_ _point_** is in a tile, which is basically dividing the tile into four smaller
  squares and checking in which one the point would be, I just decided to use the formula to find _x_ and _y_ using
  a higher _zoom_ in the first place, instead of using the real _zoom_.

  $ \\left(zoom_{higher} = zoom_{real} + 1\\right) $

  If we have an (_x_, _y_) location using a higher zoom, finding the real (_x_, _y_) values of a smaller _zoom_ is 
...
`,
  ...
  $\\left( x_{higher}, y_{higher} \\right)$, with the coordinates of the smaller squares and we can proceed to the 
  next step.
`,
  `
...
`,
  ...
  But, as stated previously, the solution should also reduce the number of API calls to a minimum.

  Therefore this solution will use 4 API calls in total for the weather layer: the one in the center and 3 others
  depending on the second step result. Taking San Francisco with a _zoom = 10_ as an example again, we discovered 
  that its **_I_ _point_** is located in the "red" quadrant of the center tile in the illustration above, so, to have
  all the information necessary to compose the full image, it's necessary to fetch the tile directly in the right,
  the one below it and the tile that is adjacent to both of them.
`,
  `
  ##### _Join the tile images in the correct order_

  This step is very straightforward, the only thing necessary is to "join" the four images with no gaps in between and
...
`,
  ...
  decompose this distance into three separate calculations: the distance between the original tile's origin point 
...
`,
  `
  The illustration below shows the **_K_ _point_** _x_ and _y_ distance values, in pixels, for each possible region
  where the **_I_ _point_** is found in step 2.
`,
  `
  ...
  But the formulas above output the horizontal and vertical location of the point inside the whole world map image
  ...
  The illustration below shows exactly that, using the city of **San Francisco, CA** again as an example with
  _**zoom = 3**_.
`,
  `
  Unfortunately, since the previous formulas to locate the interest point is referenced in the top-left corner
  of the "world image", it's necessary to change this reference to a known point inside of the image formed in the
  previous step - and there's no better point than **_K_** (if you forgot, the up-left point of the image) for
  ...
  _zoom = 3_, the position of **_I_** inside its original tile will be:

  $position_{x} \\bmod 256 = 327 \\bmod 256 = 71$ pixels

  $position_{y} \\bmod 256 = 791 \\bmod 256 = 23$ pixels

  Now, coming back to the image formed in the _Fourth Step_, the sum of the **_K_** position and the rest of the
...
`,
  `
  ##### _Crop the result with the exact offset with the **I point** in the center of the original tile_
  ...
  The illustration below uses San Francisco again as an example, and the resulting image (highlighted area) should be
...
`,
  `
  Below, one last example of San Francisco with a _zoom = 8_ and a **_wind_ weather layer** from a specific time. 
`,
  `
  That's it! 
`,
  // #### Conclusion
  `
  Now you know how I solved the Map Coordinates Problem. In general, I think this is a typical case of something that
  the user will take for granted, and may not be obvious to beginner developers how much of a problem there is to
  ...
  how I made modifications to the _weather layers'_ original colors to be even more interesting than this one. Have a
  great day!
`
];
