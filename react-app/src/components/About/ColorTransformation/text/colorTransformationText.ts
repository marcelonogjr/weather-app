...
`,
  ...
  speed up to 20 mph and then values above that and up to 75 mph - this value representing "Hurricane Force",
  ...
  determined time and location), I felt the app could benefit from having a more comprehensible scale.
...
`,
  ...
  Green, and Blue - to form every other color. The illustration below shows some examples of colors and their
  corresponding _RGB_ values.
`,
  ...
  there's also an "Alpha" parameter that sets the opacity of a given color, with 0 being fully transparent and 255
...
`,
  ...
  is responsible for correlating two or more variables. More specifically, a linear function will _always_ have all
  variables that it correlates with an [exponent](https://en.wikipedia.org/wiki/Exponentiation) equal to 1. So, if
  there's a linear function that expresses the value of _y_ in relation to _x_, it will _always_ be possible to 
  represent this function as:
  
  $y = ax + b$ , with $a \\neq 0$

  In this scenario, "a" and "b" will always be _constant_ numbers.  Graphically, assuming that _x_ and _y_ can assume
  any real number value, a linear function will always be a "straight" line (but never vertical). The next graph
  shows an example of a simple linear function, with _a_ and _b_ being equal to _2_ and _3_, respectively, and some
...
`,
  ...
  // #### Linear Gradient
  `
  Now to the last fundamental concept required for the explanation of how to solve the main problem, linear color
  gradients utilize the two previous concepts - how colors are formed in the RGBA system (in our specific case, it
  can use any color model) and linear functions. I obviously didn't explain what [gradients](https://en.wikipedia.org/wiki/Gradient)
  are, but if you don't know it already, I'm sure you'll understand it anyway.

  A linear color gradient is basically the representation of a blend of colors. It will always have at least 2 stopping
  ...
  the RGBA color model each color is formed by four different parameters that go from 0 to 255, if a function starts
...
`,
  ...
  chose that the colors would be opaque on both ends, the alpha parameter will also have a constant value (255).

  Now that every necessary concept has been explained, it's possible to understand the problem in depth
...
`,
  ...
  `
  The original scale value and the way color varies in both layers are _"satisfactory"_. The _temperature_ layer has a bit
  of a problem since every value above 86°F (30°C) is displayed at the same "maximum" color, but unfortunately, I
  cannot do anything about that. There is no way to widen the range of the original scale since every data that falls
  out of the maximum or minimum values of the scale will be presented to me in the same colors, making it impossible
  to distinguish anything that exceeds that range. So the only thing I can do is modify what happens between the
  maximum and minimum values of the original scales.
  
  The scale of the other three layers, especially the _precipitation_ and _wind speed_, is not that satisfying to me -
  the latter was already commented on in the [Introduction](#intro) section of this article. So for the three layers
  that I have modified, I think it's best to start explaining the modifications on the _clouds_ layer, since it's the
  one that has the simplest logic to modify the original colors, in my chosen solution.
  
  From now on, every time that I'm changing the colors of a layer since all the possible theoretical values are known,
...
`,
  ...
  Just to give another example, if my original color is _blue (0, 0, 255, 155)_, its complementary color is _yellow
...
`,
  ...
  weather layers it's to allow a [qualitative](https://en.wikipedia.org/wiki/Qualitative) evaluation, not a
...
`,
  ...
  I won't bore you with all the function values of the original scales or the ones that I used to transform all the values,
...
`,
  ...
  its scale values. Winds that reach over 75 mph are considered to be **really fast** winds. So for this layer, I
  ...
  additional stopping point at 22.37 mph (10 m/s). For the colors, I chose to use really simple colors that have either
...
`,
  ...
  smaller values be better represented - they're the most common values and probably what the user will see the most
  when using this app. When the precipitation (in form of liquid rain, just to be clear) reaches
  values higher than 0.9 in/3h, it is considered to be [**heavy rain**](https://glossary.ametsoc.org/wiki/Rain),
  and it's represented with a "reddish" tone in the new scale.
`,
  ...
  linear functions every consecutive stop but will have a different _y_ for the same _x_ corresponding value, as
  follows.
  ...
  The function above will be applied for each of the four channels of the RGBA, so for both layers that I decided to
  change, every channel will have distinct transformation functions depending on the correct _x_ parameter inferred in
  step two.

  That's it for the color transformation logic of the _wind speed_ and _precipitation_ layers. But when I was
...
`,
  ...
  If you are familiar with the tile coordinate system (explained in the [Necessary Concepts of the Map Coordinates
  article](./map-tiles#necessary-concepts)), you'll know that the image that is provided on the weather API has a
  constant pixel size, but the information contained a specific weather layer will depend on an x, a y, and most
  ...
  "piece" having the same size of 256x256 pixels, always. An increase of 1 in the zoom will provoke each tile to be
  divided by 2 in each dimension, therefore dividing a single tile into other 4 tiles contained inside the original
  tile. But what does that have to do with scaling?

  There are two ways to scale an image: upscale and downscale. That means to increase or decrease its size by a certain
...
`,
  // Image 12
  `
  When downscaling, the chances of losing or having to transform information are really high. 
  ...
  to have a more detailed information. Even if the image is the same, it's possible to create that new image, in the
  same way, shown in the upscaling illustration, aggregating groups of 4 pixels to be equivalent to 1 pixel of the
  original image.
  ...
  [specific OpenWeatherMap's API](https://openweathermap.org/map_legend) uses the [Lanczos](https://en.wikipedia.org/wiki/Lanczos_resampling)
  method - [this link](https://gis.stackexchange.com/questions/10931/what-is-lanczos-resampling-useful-for-in-a-spatial-context)
  provides a really good basic explanation of how this and other methods work, I really recommend you to check that
  out if are curious about how to "solve" this problem.

  The Lanczos method explains why I was getting those colors that weren't supposed to exist. As a result to that, every
  color that is located in an intermediate position in relation to other "real" pixel colors will have an unexpected
  value for probably all four channels, creating colors that I also didn't predict in [step 3](#step-three).
  The modifications that I made on the _clouds_ layer won't suffer from that problem, since I applied the same rule
  for the entire scale, converting the _modified_ pixels using the absolute same formula as everything else.
  
  At the moment that I'm writing this article, I was able to capture a [typhoon](https://en.wikipedia.org/wiki/Typhoon)
  image that can serve as a good example of how the original _wind speed_ layer was and how it got transformed by
...
`,
  ...
  there are some lines around regions with defined speed values in the modified version that are not apparent in the
  ...
  to mine. Any filter that I decide to implement in the final image will also have a loss of data. By the end of the day,
...
`,
  // ### Conclusion
  `
  That was a long ride... I hope it's been insightful even if you are not trying to replicate or adapt this solution to
